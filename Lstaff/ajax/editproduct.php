...
if($_SERVER['REQUEST_METHOD'] == "GET" and isset($_GET['pid']) and ctype_digit($_GET['pid'])){
	...
	if(!isset($_SESSION['_token']) OR !isset($_GET['token']) OR $_GET['token'] != $_SESSION['_token']){
		...
		$stmt=$conn->prepare("SELECT name FROM products WHERE id=:user");
...
if($_SERVER['REQUEST_METHOD'] == "POST"){
	...
	} else if(isset($_POST['name'],$_POST['id'],$_POST['dis'],$_POST['price'],$_POST['type'],$_POST["file"])){
		if(antiSpam("editproduct:editproduct.php")){
			returnJSON(array('t'=>'خطأ','m'=>'من فضلك انتظر قليلا ثم حاول مجدداً', 's'=>'error', 'b'=>'موافق'));
		}
$filename = '../../Lstorage/'.$_POST['file'].'';

	if(empty($_POST['name']) OR empty($_POST['dis']) OR empty($_POST['price']) OR empty($_POST['type']) OR empty($_POST['file'])){
		...
		$stmt->execute();
				...
				$stmt2=$conn->prepare("SELECT id FROM products WHERE name=:name");
...
$sql = "UPDATE products SET name=:name,type=:type,describes=:describes,price=:price,file=:file,enc=:enc WHERE id=:id";
